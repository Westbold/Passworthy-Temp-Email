...
/**
 * Get a permanent address claim by email address
 */
export async function getClaimByEmailAddress(db: D1Database, emailAddress: string) {
	try {
		const result = await db
			...
			.bind(emailAddress)
			.first();
		return { result: result as Claim | null, error: undefined };
	} catch (e: unknown) {
		const error = e instanceof Error ? e : new Error(String(e));
